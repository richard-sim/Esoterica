...
#include "RenderContext_Vulkan.h"
...
namespace EE::Render
{
    class EE_SYSTEM_API VRenderDevice
    {

    public:

        VRenderDevice() = default;
        ~VRenderDevice();
        ...
        inline RenderContext const& GetImmediateContext() const
        {
        ...
        RenderTarget const* GetPrimaryWindowRenderTarget() const
        {
            return &m_primaryWindow.m_renderTarget;
        }
        RenderTarget* GetPrimaryWindowRenderTarget()
        {
            return &m_primaryWindow.m_renderTarget;
        }
        inline Int2 GetPrimaryWindowDimensions() const
        {
            return m_resolution;
        }
        ...
        inline void CreateDataTexture( Texture& texture, TextureFormat format, Blob const& rawData )
        {
            CreateDataTexture( texture, format, rawData.data(), rawData.size() );
        }
